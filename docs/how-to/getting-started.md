...
Change key/model later: `nodedex setup --key sk-or-… --model vendor/model` (headless), or
`nodedex onboard` for the wizard. Remove everything (config + graphs, not the code): `nodedex uninstall`.
